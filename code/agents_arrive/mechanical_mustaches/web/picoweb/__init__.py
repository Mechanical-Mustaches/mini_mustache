...
import mechanical_mustaches.web.pkg_resources
...
class WebApp:
    ...
    def _load_template(self, tmpl_name):
        if self.template_loader is None:
            ...
            self.template_loader = utemplate.source.Loader(None, "templates")
            # ANFRO EDIT!!!! # self.template_loader = utemplate.source.Loader(self.pkg, "templates")
    ...
    def run(self, host="127.0.0.1", port=8081, debug=False, lazy_init=False, log=None):
        if log is None and debug >= 0:
            import mechanical_mustaches.web.ulogging
            log = mechanical_mustaches.web.ulogging.getLogger("picoweb")
            if debug > 0:
                log.setLevel(mechanical_mustaches.web.ulogging.DEBUG)
        self.log = log
        gc.collect()
